...
```{r message=FALSE, echo=FALSE, results='hide'}
colours<-c("#e7e1ef","#d4b9da","#c994c7","#df65b0","#e7298a","#ce1256","#980043","#67001f")
scatter1<- function(df,factor1,i){
...
# b<-paste(l[i],a[7]," with ","t value equal to",round(as.numeric(a[1]),3),", 95 percent confidence interval is",a[9],"with a correlation of",round(as.numeric(a[4]),3)) 
 ...
 p0<-ggplot(aes(df[,factor1], df1[,i], color = factor(quality)), data = df1) +    geom_jitter(size = 2,alpha=0.8) +
  scale_color_brewer(type='seq', 
    ...
    ylab(m[i])+  theme(panel.background = element_rect( fill = "grey90"))
...
p1<-ggplot(aes(df[,factor1], df1[,i], color =factor(quality)),data=df1) +
  ...
  geom_smooth(se = FALSE) +
    xlab(factor1) +
    ylab(m[i]) +  theme(panel.background = element_rect( fill = "grey85"))
...
# ?geom_jitter
##scatter(redwines$alcohol, "alcohol", redwines$volatile.acidity, "volatile acidity",11)
scatter1(redwines, "alcohol",10)
lapply(1:10, function(i) scatter1(redwines, "alcohol",i))
lapply(1:10,function(i) lapply(i:10, function(j) scatter1(redwines, names(redwines)[i],j)))
# i=4
# lapply(i:10, function(j) scatter1(redwines, names(redwines)[i],j))
