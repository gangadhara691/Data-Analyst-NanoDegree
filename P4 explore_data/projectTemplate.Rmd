...
Boxinscatter <- function(x,i,l,factor, na.rm = TRUE, ...) {
  ...
  p1 <-ggplot(aes(factor(x[,factor]), x[,i]), data = x) +
    ...
    geom_hline(yintercept = (median(x[,i]) + sd(x[,i])*3.5),linetype = 4) +
    ylab(m[i])+xlab(factor)+theme(text = element_text(size=15))
  
  a<-paste(with(data=x,cor.test(x[,i],x[,factor])))
  b<-paste(l[i],a[7]," with ","t is equal to",round(as.numeric(a[1]),3),", 95 percent confidence interval is",a[9],"with a correlation of",round(as.numeric(a[4]),3))
...
#a<-paste(with(data=x,cor.test(x[,i],x[,factor])))
commentz<-list("There positive correlation between fixed acidity and quality. The outliers fall in wine of quality 7 and 5, and none feel in the lowest quality wines."
               ,"We can see Negitive correlation in the trend here."
               ...
               ,"A weak negitive correlation is seen.Trend between quality and less density goes down."
               ...
               ,"A very high correlation is seen ,higher alcohol content will give high quality "
...
#Boxinscatter(redwines,2,commentz,'quality')
lapply(1:12, function(i) Boxinscatter(redwinesx,i,commentz,'quality'))
...
```{r message=FALSE, echo=FALSE, results='hide'}
commentz<-list("It's evident in the graph that for majority of values of chlorides(from 0.07 to 0.09), the density is positively correlated with chlorides."
...
```{r message=FALSE, echo=FALSE, results='hide'}
commentz<-list("The pH is positively correlated with the volatile.acidity."
...
```{r message=FALSE, echo=FALSE, results='hide'}
commentz<-list("citric.acid is inversely related with volatile.acidity. It might be the case that during wine preparation, not too much volatile acidity will be added if there is already good amount of fixed acidity present due to citric acid. This will help keeping the total acidity of wine to acceptible level."
